...
export default function FormViewPage() {
	...
	const [form, setForm] = useState<(FormTemplate & { id: string }) | null>(
		null,
	);
	...
	useEffect(() => {
		const loadForm = async () => {
			...
			try {
				const formData = await getFormTemplate(
				...
				<Card className='border-border bg-card max-w-md w-full mx-4'>
					<CardContent className='py-12'>
						<div className='text-center'>
							...
							<p className='text-muted-foreground'>
								The form you&apos;re looking for doesn&apos;t
								exist
							</p>
							<Link
...
></iframe>`;
					...
					<TabsContent value='overview' className='space-y-4'>
						<Card className='border-border bg-card'>
							<CardHeader>
								...
								<CardDescription>
									The database structure and input validation
								...
								<div className='space-y-3'>
									{form.fields
										...
										.map((field, idx) => (
											<div
												key={field.id}
												className='p-4 rounded-lg border border-border bg-muted/20 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3'>
												<div className='space-y-1'>
													...
													<div className='flex flex-wrap items-center gap-3 text-xs text-muted-foreground pt-1'>
														<span>
															Key:{" "}
															<code className='font-mono text-white bg-slate-800 px-1 rounded'>
																{field.name}
															</code>
														</span>
						...
						<div className='grid grid-cols-1 lg:grid-cols-12 gap-6'>
							...
							<div className='lg:col-span-4 space-y-6'>
								<Card className='border-border bg-card'>
									<CardHeader className='pb-3 border-b border-border'>
										...
										<CardDescription>
											Modify form appearance parameters
											for your website landing page.
										</CardDescription>
									</CardHeader>
									<CardContent className='pt-4 space-y-6'>
										...
										<div className='flex items-center justify-between p-2 rounded-lg border border-border/50 bg-muted/10'>
											<div className='space-y-0.5'>
												<label
													htmlFor='bg-trans'
													className='text-xs font-semibold uppercase tracking-wider text-muted-foreground block cursor-pointer'>
													Transparent BG
												</label>
							...
							<div className='lg:col-span-8 space-y-4'>
								<div className='flex items-center justify-between'>
									...
									<span className='text-[10px] text-muted-foreground'>
										Simulated render of customizations. No
										data is stored on preview submit.
									</span>
								</div>
								...
								{deviceMode === "mobile" ? (
									...
									<div className='mx-auto max-w-[340px] border-[10px] border-slate-800 rounded-[36px] overflow-hidden bg-slate-950 shadow-2xl relative transition-all duration-300'>
										...
										<div
											className={`p-4 h-[480px] overflow-y-auto ${theme === "light" ? "bg-slate-50" : "bg-slate-950"}`}>
											{renderMockupForm()}
										</div>
									</div>
								) : (
									...
									<div className='border border-border rounded-lg overflow-hidden bg-slate-950 shadow-2xl transition-all duration-300'>
										...
										<div
											className={`p-8 h-[460px] overflow-y-auto ${theme === "light" ? "bg-slate-50" : "bg-slate-950"}`}>
						...
						<div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
							...
							<div className='space-y-6'>
								<Card className='border-border bg-card'>
									<CardHeader>
										...
										<CardDescription>
											Send this URL directly to leads or
								...
								<Card className='border-border bg-card'>
									<CardHeader>
										...
										<CardDescription>
											Paste this HTML block inside your
											page builder (WordPress, Webflow,
											Shopify, custom HTML).
										</CardDescription>
									</CardHeader>
									<CardContent className='space-y-4'>
							...
							<div className='space-y-6'>
								<Card className='border-border bg-card'>
									<CardHeader>
										<CardTitle className='text-lg'>
											How to Embed in Common Builders
										</CardTitle>
									</CardHeader>
									<CardContent className='space-y-5 text-sm text-muted-foreground leading-relaxed'>
										<div className='space-y-1.5'>
											...
											<p className='text-xs'>
												Add a custom **HTML Widget** or
												block inside Elementor /
												Gutenberg and paste the iframe
												embed code.
											</p>
										</div>
										...
										<div className='space-y-1.5'>
											...
											<p className='text-xs'>
												Add an **Embed element** inside
												your layout panel, paste the
												code, and set its height
												parameters to responsive.
											</p>
										</div>

										<div className='space-y-1.5'>
											<h4 className='font-semibold text-foreground text-xs uppercase tracking-wider'>
												HTML/CSS Custom Landing Pages
											</h4>
											<p className='text-xs'>
												Paste the iframe tag directly
												inside any container div. We
												recommend toggling **Transparent
												BG** so that the form inherits
												the page background dynamically.
											</p>
										</div>
										...
										<div className='p-3.5 rounded-lg bg-primary/5 border border-primary/20 text-xs text-primary flex items-start gap-2.5'>
											...
											<div>
												<span className='font-semibold block mb-0.5'>
													Quick Design Pro Tip:
												</span>
												Adjust settings in the **Design
												& Preview** tab before copying.
												The link and iframe codes
												auto-update in real-time with
												custom values.
											</div>
										</div>
									</CardContent>
	...
	function renderMockupForm() {
		...
		if (previewSubmitted) {
			return (
				<div
					className={`p-6 text-center space-y-4 rounded-xl ${wrapperBg} animate-in fade-in zoom-in-95 duration-200`}>
					...
					<h3
						className={`text-lg font-bold ${isDarkTheme ? "text-white" : "text-slate-900"}`}>
						Form submitted successfully!
					</h3>
					<p
						className={`text-xs ${isDarkTheme ? "text-slate-400" : "text-slate-500"}`}>
						(This is a simulated submission preview. No records were
		...
		return (
			<div className={`p-6 rounded-xl space-y-4 ${wrapperBg}`}>
				<div className='space-y-1.5 pb-2 text-center'>
					<h3
						className={`text-xl font-bold tracking-tight ${isDarkTheme ? "text-white" : "text-slate-950"}`}>
						{form.name}
					</h3>
					{form.description && (
						<p
							className={`text-xs leading-relaxed ${isDarkTheme ? "text-slate-400" : "text-slate-500"}`}>
							{form.description}
						</p>
					)}
				</div>
