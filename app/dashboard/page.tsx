...
import { TrendingUp, FileText, Users, Activity, Menu } from "lucide-react";
		...
		const loadData = async () => {
			...
			try {
				...
				leads.forEach((lead) => {
					...
					const chartDay = last7Days.find(
						(item) => item.dateStr === leadDateStr,
					);
					if (chartDay) {
						chartDay.count++;
					}
				...
				<Card className='border-border bg-card'>
					<CardHeader className='p-4 sm:p-6 pb-2'>
						<CardTitle className='text-base sm:text-lg'>
							Lead Collection Trend
						</CardTitle>
						<CardDescription className='text-xs'>
							Leads collected over the last 7 days
						</CardDescription>
					</CardHeader>
